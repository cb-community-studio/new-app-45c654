...
import author-infoFakerFactory from "./author-infoFakerFactory";
...
const Author-infoFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = author-infoFakerFactory(fakerCount);
        let results = [];
        setLoading(true);
        try {
            for (let i = 0; i < fakeData.length; i++) {
                const _data = fakeData[i];
                const res = await client.service("author-info").create(_data);
                // const res = _data;
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Author-infoFakerDialogComponent);
