...
import user-infoFakerFactory from "./user-infoFakerFactory";
...
const User-infoFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = user-infoFakerFactory(fakerCount);
        let results = [];
        setLoading(true);
        try {
            for (let i = 0; i < fakeData.length; i++) {
                const _data = fakeData[i];
                const res = await client.service("user-info").create(_data);
                // const res = _data;
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(User-infoFakerDialogComponent);
